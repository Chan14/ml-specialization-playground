...
def compute_entropy_test(target):
    ...
    assert np.isclose(
        target(y), 0.918295, atol=1e-6
    ), "Wrong value. Something between 0 and 1"
...
def split_dataset_test(target):
    ...
    # Case 1
    X = np.array([[1, 0], [1, 0], [1, 1], [0, 0], [0, 1]])
    X_t = np.array([[0, 1, 0, 1, 0]])
    X = np.concatenate((X, X_t.T), axis=1)

    left, right = target(X, list(range(5)), 2)
    expected = {"left": np.array([1, 3]), "right": np.array([0, 2, 4])}

    assert type(left) == list, f"Wrong type for left. Expected: list got: {type(left)}"
    assert (
        type(right) == list
    ), f"Wrong type for right. Expected: list got: {type(right)}"

    assert (
        type(left[0]) == int
    ), f"Wrong type for elements in the left list. Expected: int got: {type(left[0])}"
    assert (
        type(right[0]) == int
    ), f"Wrong type for elements in the right list. Expected: number got: {type(right[0])}"
    ...
    ), f"Wrong value for right. Expected: { expected['right']} \ngot: {right}"
    assert np.allclose(
        left, expected["left"]
    ), f"Wrong value for left. Expected: { expected['left']} \ngot: {left}"

    # Case 2
    X = np.array([[0, 1], [1, 1], [1, 1], [0, 0], [1, 0]])
    X_t = np.array([[0, 1, 0, 1, 0]])
    ...
    assert len(left) == 2, f"left must have 2 elements but got: {len(left)}"
    assert len(right) == 3, f"right must have 3 elements but got: {len(right)}"
    assert np.allclose(right, expected["right"]) and np.allclose(
        left, expected["left"]
    ...
    expected = {"left": np.array([1, 3, 6]), "right": np.array([2, 7, 9, 10])}
...
def compute_information_gain_test(target):
    ...
    ), f"Information gain must be 0 when target variable is pure. Got {result1} and {result2}"
    ...
    assert np.isclose(
        result, 0.019973, atol=1e-6
    ), f"Wrong information gain. Expected {0.019973} got: {result}"

    result = target(X, y, node_indexes, 1)
    assert np.isclose(
        result, 0.170951, atol=1e-6
    ), f"Wrong information gain. Expected {0.170951} got: {result}"

    node_indexes = list(range(4))
    result = target(X, y, node_indexes, 0)
    assert np.isclose(
        result, 0.311278, atol=1e-6
    ), f"Wrong information gain. Expected {0.311278} got: {result}"

    result = target(X, y, node_indexes, 1)
    assert np.isclose(
        result, 0, atol=1e-6
    ), f"Wrong information gain. Expected {0.0} got: {result}"
...
def get_best_split_test(target):
    ...
    assert (
        result == -1
    ), f"When the target variable is pure, there is no best split to do. Expected -1, got {result}"

    y = X[:, 0]
    result = target(X, y, node_indexes)
    assert (
        result == 0
    ), f"If the target is fully correlated with other feature, that feature must be the best split. Expected 0, got {result}"
    y = X[:, 1]
    result = target(X, y, node_indexes)
    assert (
        result == 1
    ), f"If the target is fully correlated with other feature, that feature must be the best split. Expected 1, got {result}"
